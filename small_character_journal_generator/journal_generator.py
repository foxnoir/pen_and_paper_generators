#!/usr/bin/env python3
"""
Dynamic PDF Generator for Character Journal
...
class DynamicPDFGenerator:
    ...
    def calculate_pages_from_structure(self, structure: Dict, cover_pages_config: Dict = None) -> Tuple[List[Dict], Dict, Dict]:
        ...
        if "right_tab_start_y" in metadata:
            self.layout_generator.right_tab_start_y = metadata["right_tab_start_y"]
        if "right_tab_spacing" in metadata:
            self.layout_generator.right_tab_spacing = metadata["right_tab_spacing"]
        ...
        start_y = metadata.get("right_tab_start_y", self.layout_generator.right_tab_start_y)
        tab_spacing = metadata.get("right_tab_spacing", self.layout_generator.right_tab_spacing)
        current_y = start_y
        
        for tab_data in right_tabs_data:
            # Read tab name directly from JSON (no hardcoded names!)
            tab_name = tab_data.get("name", "")
            height = self.layout_generator.compute_right_tab_height(tab_name)
            ...
            if tab_name == "Impressionen":
                tab_page_count = self.layout_generator.impressionen_tab_page_count()
            ...
            for subsection in subsections:
                ...
                # Then pages for each sub-subsection (flat or nested groups)
                subsection_config = {}
                if sub_subsections_config:
                    tab_config = sub_subsections_config.get(tab_name, {})
                    subsection_config = tab_config.get(subsection_name, {})

                for entry in LayoutGenerator.parse_sub_subsection_entries(sub_subsections):
                    if entry["children"]:
                        group_name = entry["name"]
                        group_config = subsection_config.get(group_name, {})
                        group_page_count = LayoutGenerator.page_count_from_config(group_config)
                        if group_page_count > 0:
                            for page_idx in range(1, group_page_count + 1):
                                page_structure[current_page] = {
                                    "tab_name": tab_name,
                                    "subsection": subsection_name,
                                    "sub_subsection": None,
                                    "sub_subsection_group": group_name,
                                    "page_index": page_idx,
                                }
                                current_page += 1
    ...
    def generate_from_json(self, json_path: str, output_path: str, source_pdf_path: str = None):
        ...
        if os.path.exists(cover_pages_path):
            ...
            with open(cover_pages_path, 'r', encoding='utf-8') as f:
                cover_pages_config = json.load(f)
            self.layout_generator.sync_impressionen_cover_pages(cover_pages_config)
            n_imp = self.layout_generator.impressions_image_count()
            print(
                f"Impressionen: {n_imp} Bilder aus assets/images/impressions/ "
                f"+ {LayoutGenerator.IMPRESSIONS_DOSSIER_EXTRA_PAGES} Dossier-Seiten"
            )
