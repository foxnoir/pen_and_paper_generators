...
<div align="center">
  ...
  <h1 align="center">SL Journal Generator</h1>

  <p align="center">
    Generate dynamic PDF journals with interactive navigation for Vampire: The Masquerade sessions
    <br />
...
The SL Journal Generator creates dynamic PDF journals for **Vampire: The Masquerade** sessions with interactive navigation tabs, blood splatter effects, and customizable content structure. Perfect for game masters who want professional-looking session journals with easy navigation.
...
- **Visual Effects**: Subtle grayscale blood splatter effects in the background
...
- Generate `SL_journal.pdf` with all tabs and content
- Extract font styling from `vampire_journal.pdf` (if available)
...
├── layout_generator.py       # Handles visual layout (tabs, splatters)
├── content_processor.py      # Handles content extraction and processing
├── tab_structure.json        # JSON configuration file
├── vampire_journal.pdf       # Source PDF (optional, for font styling)
├── SL_journal.pdf           # Generated output
...
- **Blood Splatters**: Grayscale clusters (2-6 per page, 4-19 spots per cluster)
- **Language**: Python 3.7+
- **Output Format**: PDF
...
Adjust splatter parameters in `layout_generator.py`:
- Number of clusters per page (default: 2-6)
- Spots per cluster (default: 4-19)
- Color saturation (default: 5-12%)
- Color ranges (light: ~#D4D3D3, dark: ~#7C7979)
