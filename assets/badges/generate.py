...
The mid stop is the brand or project color. Official colors stay official
...
BADGES: list[tuple[str, str, str, str]] = [
    ...
    ("pymupdf", "PyMuPDF", "#02569B", "blue (PDF tooling)"),
    ("pdf", "PDF", "#4A8C6F", "green (output format)"),
    ("png", "PNG", "#5E6AA8", "blue-violet (output format)"),
    ("macos", "macOS", "#4DB8C4", "pastel turquoise (replaces black)"),
    ("linux", "Linux", "#2A9D8F", "turquoise (replaces yellow/black)"),
    ("windows", "Windows", "#0175C2", "blue (platform)"),
...
def badge_svg(slug: str, label: str, mid: str) -> str:
  ...
  <rect width="{width}" height="28" fill="url(#{fill_id})"/>
...
def write_all(out_dir: Path) -> None:
    ...
    for slug, label, mid, _note in BADGES:
        (out_dir / f"{slug}.svg").write_text(badge_svg(slug, label, mid), encoding="utf-8")
